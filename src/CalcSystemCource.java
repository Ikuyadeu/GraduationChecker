...
public class CalcSystemCource extends CalcGraduation {
	...
	String judgeGraduation(Person p) {
		...
		if(calcCreditSum(p)<calcNeedSum()){
			judge += "基礎科目、教養育成科目及び専門基礎科目が"+ (calcNeedBasic() - calcCreditBasic(p))+"単位足りません。\n";
		}
		
		if(calcCreditSum(p)<calcNeedSum()){
			judge += "専門教育科目　専門必修（コース別）欄の科目が"+ (calcNeedSpeciallty() - calcCreditSpecilallty(p))+"単位足りません。\n";
		}
	...
	private int calcCreditSum(Person p){
		return p.getBasic() + p.getLiberal() + p.getCompulsory() + p.getSpecialltyBasic() + p.getSpecialltyEducation() ;
	}
